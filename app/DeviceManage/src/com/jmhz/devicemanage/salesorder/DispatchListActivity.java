...
import com.jmhz.devicemanage.adapter.DispatchAdapter;
...
public class DispatchListActivity extends Activity implements
		OnItemClickListener, OnScrollListener {
	private ListView mPayments = null;
	private DispatchAdapter mAdapter = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_salas_dispatch_list);
		mPayments = (ListView) findViewById(R.id.dispatch_list);
	...
	private void initListView() throws Exception {
		...
		od.setPaging(p);
		od.setOrderStatus(OrderStatus.HAS_DISPATCHING.ordinal() + "");
		getList(od);
		...
		new SalesOrderApi(this).getListByStatus(od, new HttpCallback() {
			...
			public void onSuccess(Object o) {
				...
				for (OrderDetail od : ods) {
					PaymentItem paymentItem = new PaymentItem(od.getOrderSn(),
							od.getOrderAddTime(), od.getOrderAmount(), "已配送");
					rowResult.add(paymentItem);

				}
				mAdapter = new DispatchAdapter(rowResult,
						DispatchListActivity.this);
				mPayments.setOnItemClickListener(DispatchListActivity.this);
				mPayments.setOnScrollListener(DispatchListActivity.this);
				...
				Toast.makeText(DispatchListActivity.this,
			...
			public void onFail(String errorCode, String errorMsg) {
				Toast.makeText(DispatchListActivity.this, errorMsg,
						Toast.LENGTH_SHORT).show();
				btn_reload.setVisibility(View.VISIBLE);
				iv_blank.setVisibility(View.VISIBLE);
			}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			long arg3) {
		Intent intent = new Intent(DispatchListActivity.this,
		...
		if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
			if (view.getLastVisiblePosition() == view.getCount() - 1) {
				if (rowTotal < pageSize) {
					Toast.makeText(DispatchListActivity.this, "没有更多数据！",
							Toast.LENGTH_SHORT).show();
					return;
				}
				...
				if (rowTotal > rowResult.size()) {
					if (!mLoading) {
						...
						Toast.makeText(DispatchListActivity.this, "加载中，请稍后！",
								Toast.LENGTH_SHORT).show();
					}
				} else {
					Toast.makeText(DispatchListActivity.this, "没有更多数据！",
	...
	private void getMoreList(OrderDetail od) throws Exception {
		new SalesOrderApi(this).getListByStatus(od, new HttpCallback() {
			...
			public void onSuccess(Object o) {
				...
				for (OrderDetail od : ods) {
					PaymentItem paymentItem = new PaymentItem(od.getOrderSn(),
							od.getOrderAddTime(), od.getOrderAmount(), "滚动加载");
					rowResult.add(paymentItem);
				}
				Toast.makeText(DispatchListActivity.this,
