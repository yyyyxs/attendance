            ...
            <tr>
                <td>
                    <select class="form-control" name="approve"
                        ...
                        <option value="1">同意升级</option>
                        <option value="2">拒绝升级</option>
                        <option value="3">已升级</option>
                    </select>
                </td>
            </tr>
            </tbody>
...
<script type="text/javascript">
            ...
            buttons: [
                ...
                },
                {
                    ...
                    click: function () {
                        $.ajax({
                            type: "POST",
                            url: "${ctx}/Upgrade/approveopinion",
                            data: {
                                id: id,
                                status: $("#status").val(),
                                approve: $("#approve").val(),
    ...
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            url: "${ctx}/Upgrade/getAllapply",
            datatype: "json",
            mtype: "GET",
            height: 'auto',
            colNames: ['id', '设备id', '设备名字', '设备类型', '设备使用者', '申报时间', '升级描述', '设备状态', '审核进度','审核意见', '升级报告', '操作'],
            colModel: [
                {name: 'id', index: 'id', width: 30, sorttype: "int", editable: false, formatter: makeURL},
                {name: 'deviceId', index: 'deviceId', width: 50, sorttype: "int", editable: false},
                {name: 'deviceName', index: 'deviceName', width: 50, editable: false},
                {name: 'deviceType', index: 'deviceType', width: 50, editable: false, formatter: styleType},
                {name: 'deviceUser', index: 'deviceUser', width: 60, editable: false},
                {name: 'applytime', index: 'applytime', width: 55, editable: false},
                {name: 'upgradeDescribe', index: 'upgradeDescribe', width: 80, editable: false},
                {name: 'status', index: 'status', width: 45, sorttype: "int", editable: true, formatter: styleStatus},
                {
                    name: 'approve',
                    index: 'approve',
                    width: 55   ,
                ...
                {name: 'approveRemark', index: 'approveRemark', width: 80, editable: false},
                {name: 'logmark', index: 'logmark', width: 50, editable: false, formatter: stylelogmark},
                {name: 'operation', index: '', width: 40, formatter: makeURLForOperation},

                /*{name: 'operation', index: '', width: 30, formatter: makeURLFor},*/
        ...
        function makeURLForOperation(cellvalue, options, rowObject) {
            var urlstring =
                    ...
                    "<button title=\"审核意见\" class=\"btn btn-xs btn-info\" style=\"margin: 0\"id=\"changePwd\" margin-left:10px;\" onclick=showApproveDialog(" + rowObject["id"] + ")> <i class=\"icon-bookmark-empty bigger-120\" style=\"margin: 0;\"></i></button>" +
                    "<button title=\"查看升级报告\" class=\"btn btn-xs btn-grey\" style=\"margin: 0;\" id=\"changePwd\" " +
                    "onclick=\"window.open('${ctx}/Upgrade/showlog/" + rowObject["id"] + "','_blank')\">" +
                    "<i class=\"icon-search bigger-120\" style=\"margin: 0;\"></i></button>" +
                    "</div>";
            return urlstring;
        }

        /*   function makeURLFor(cellvalue, options, rowObject) {
         var urlstring =
         "<div class=\"btn-group\" id='operationBtn'>" +
         "<button title=\"升级报告\" class=\"btn btn-xs btn-info\" style=\"margin: 0;\" id=\"changePwd\" " +
         "onclick=\"window.open('
        ${ctx}/Upgrade/upgradeLog" + rowObject["id"] + "','_blank')\">" +
         "<i class=\"icon-search bigger-120\" style=\"margin: 0;\"></i></button>" +
         "</div>";
         return urlstring;
         }*/
