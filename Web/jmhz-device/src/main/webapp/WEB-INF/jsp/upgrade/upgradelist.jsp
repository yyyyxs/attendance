...
<script type="text/javascript">
    $(function ($) {
        ...
        $(grid_selector).jqGrid({
            ...
            mtype: "GET",
            height: 'auto',
            colNames: ['id', '设备id', '设备名字', '设备类型', '设备使用者', '申报时间', '升级描述', '设备状态', '审核进度','审核意见', '升级报告', '查看'],
            colModel: [
                ...
                {name: 'deviceName', index: 'deviceName', width: 54, editable: false},
                {name: 'deviceType', index: 'deviceType', width: 40, editable: false, formatter: styleType},
                {name: 'deviceUser', index: 'deviceUser', width: 40, editable: false},
                {name: 'applytime', index: 'applytime', width: 50, editable: false},
                {name: 'upgradeDescribe', index: 'upgradeDescribe', width: 56, editable: false},
                ...
                {name: 'operation', index: 'operation', width: 40, formatter: makeURLForOperation},
            ],
            viewrecords: true,
            rowNum: 10,
            rowList: [10, 20, 30],
        ...
        function makeURLForOperation(cellvalue, options, rowObject) {
            var urlstring =
                    ...
                    "<button title=\"添加升级日志\" class=\"btn btn-xs btn-success\" style=\"margin: 0;margin-left:10px;\" " +
...
<script type="text/javascript">
    function viewCondition(id, approve) {
        if (approve == "0" || approve == "2") {
            ...
            window.open("${ctx}/Upgrade/modifyapply/" + id);

        }
        else {
            bootbox.dialog({
                message: "<span class='bigger-110'>你的申报已经通过或者设备已升级完成</span>",
                buttons: {
                    "success": {
...
<script type="text/javascript">
    function addlog(id, approve, logmark) {
        ...
        if (approve == "1" || approve == "3") {
            ...
            if (logmark == "1") {
                bootbox.dialog({
                    message: "<span class='bigger-110'>已有升级报告，不能再次添加！</span>",
                    buttons: {
                        "success": {
                            "label": "<i class='icon-ok'></i> 确定",
                            "className": "btn-sm btn-success"
                ...
                window.open("${ctx}/Upgrade/upgradeLog/" + id);
            }

        }
        else {
            bootbox.dialog({
                message: "<span class='bigger-110'>未通过审核，你不能填写升级报告！</span>",
                buttons: {
                    "success": {
                        "label": "<i class='icon-ok'></i> 确定",
                        "className": "btn-sm btn-success"
