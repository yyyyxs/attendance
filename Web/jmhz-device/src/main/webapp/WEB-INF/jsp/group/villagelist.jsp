...
<html lang="en">
    ...
    <body>
                    ...
                    <div class="page-content">
                        <div class="row">
                            <div class="col-xs-12">
                                ...
                                <div id="addVisitDialog" class="hide">
                                    ...
                                    <table width="100%">
                                        ...
                                        <tr>
                                            <td>
                                                <input class="date-picker col-xs-5" id="visittime" name="visittime" type="text"
        ...
        <script type="text/javascript">
            ...
            <%--function showAddVisitDialog(id) {--%>
                <%--$("#addVisitDialog").removeClass('hide').dialog({--%>
                    <%--height: 300,--%>
                    <%--width: 600,--%>
                    <%--resizable: false,--%>
                    <%--modal: true,--%>
                    ...
                    <%--buttons: [--%>
                        ...
                        <%--},--%>
                        <%--{--%>
                            <%--html: "<i class='icon-ok bigger-110'></i>&nbsp; 添加走访记录",--%>
                            <%--"class": "btn btn-success btn-xs",--%>
                            <%--click: function () {--%>
                                <%--$.ajax({--%>
                                    <%--type: "POST",--%>
                                    <%--url: "${ctx}/farmerVisit/addVisit",--%>
                                    <%--data: {--%>
                                        <%--farmerid: id,--%>
                                    ...
                                    <%--success: function (data) {--%>
                                        <%--if (data.success) {--%>
                                            ...
                                            <%--}).trigger("reloadGrid");--%>
                                            <%--bootbox.dialog({--%>
                                                <%--message: "<span class='bigger-110'>农户走访时间新增成功！</span>",--%>
                                                <%--buttons: {--%>
                                                    <%--"success": {--%>
                                        ...
                                        <%--} else {--%>
                                            <%--bootbox.dialog({--%>
                                                <%--message: "<span class='bigger-110'>农户走访时间新增失败！请联系系统管理员解决问题.</span>",--%>
                                                <%--buttons: {--%>
                                                    <%--"success": {--%>
                                                        <%--"label": "<i class='icon-ok'></i> 确定",--%>
                                                        <%--"className": "btn-sm btn-success",--%>
            ...
            $(function($) {
                ...
                $(grid_selector).jqGrid({
                    url : "${ctx}/group/getAllVillage",
                    datatype: "json",
                    mtype: "GET",
                    height: 372,
                    colNames:['ID','镇名','村名','村名小组','户数','党员小组数','党员人数','编辑'],
                    colModel:[

                        {name:'id',index:'id', width:40, sorttype:"int", editable: false},
                        {name:'town',index:'town', width:40, sorttype:"int", editable: false},
                        {name:'village',index:'village',width:30, editable:false, sorttype:"textarea",
                            formatter: makeURL
                        },
                        {name:'villagerTeam',index:'villagerTeam', width:22,editable: true , editable: false},
                        {name:'households',index:'households', width:30, sorttype:"int", editable: false},
                        {name:'partyGroupCnt',index:'partyGroupCnt', width:25, sorttype:"int", editable: false},
                        {name:'partyMemCnt',index:'partyMemCnt', width:25, sorttype:"int", editable: false},
                    ...
                    editurl: "${ctx}/group/villagemodify",//nothing is saved
                    caption: "村情信息",
                    autowidth: true,
                ...
                function makeURL(cellvalue, options, rowObject) {
                    ...
                    var urlstring;
                    urlstring = "<a href='${ctx}/group/villagetoviewpage/"+rowObject["id"]+"' target='_blank'>"+cellvalue+"</a>";
                    return urlstring;
                }
                ...
                function exportExcelDataByFilter() {
                    $.ajax({
                        ...
                        success: function (data) {
                            $("#maskDIV").fadeOut();
                            bootbox.dialog({
                                message: "<span class='bigger-110'>村情信息正在导出为Excel，请稍后点击导出列表查看下载.</span>",
                                buttons: {
                                    "success": {
                                        "label": "<i class='icon-ok'></i> 确定",
                                        "className": "btn-sm btn-success",
                        ...
                        error: function (XMLHttpRequest, errorThrown) {
                            $("#maskDIV").fadeOut();
                            bootbox.dialog({
                                message: "<span class='bigger-110'>村情信息导出失败！请联系系统管理员解决问题.</span>",
                                buttons: {
                                    "success": {
                                        "label": "<i class='icon-ok'></i> 确定",
                                        "className": "btn-sm btn-success",
