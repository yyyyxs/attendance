        ...
        <script type="text/javascript">
            $(function($) {
                ...
                $(grid_selector).jqGrid({
                    url : "${ctx}/smhlappeal/getAllOriginalAppeal",
                    datatype: "json",
                    mtype: "GET",
                    height: 'auto',
                    colNames:['诉求编号','诉求号码','诉求内容','状态','接收日期','诉求操作'],
                    colModel:[
                        {name:'id',index:'id', width:10, sorttype:"int", editable: false, hidden: true},
                        {name:'tel',index:'tel', width:20, editable: false},
                        {name:'content',index:'content', width:150, editable: false , sorttype:"textarea"},
                        {name:'status',index:'status',width: 15, editable:false, formatter: styleStatus},
                        {name:'createtime',index:'createtime',width: 15, editable:false, sorttype:"textarea"},
                        {name:'operation',index:'', width:25, formatter: makeURLForOperation}
                    ],
                    viewrecords : true,
                    rowNum:10,
                    rowList:[10,20,30],
                ...
                function makeURLForOperation(cellvalue, options, rowObject) {
                    var urlstring =
                            ...
                            "<button title=\"查看诉求\" class=\"btn btn-xs btn-info\" style=\"margin: 0;\" id=\"modRoleBtn\" " +
                                    "onclick=\"window.open('${ctx}/smhlappeal/tovieworiginal/" + rowObject["id"]+"','_blank')\">" +
                                    "<i class=\"icon-search bigger-120\" style=\"margin: 0;\"></i></button>" +
                            "<button title=\"诉求受理\" class=\"btn btn-xs btn-grey\" style=\"margin: 0;margin-left:10px;\" id=\"changePwd\" " +
                                    "onclick=\"window.open('${ctx}/smhlappeal/originalTransformAppeal/" + rowObject["id"] + "','_blank')\">" +
                    ...
                    buttons: [
                        {
                            ...
                            click: function () {
                                $.ajax({
                                    type: "POST",
                                    url: "${ctx}/smhlappeal/deleteOriginal",//原本是modify，现在改用新方法delete
                                    data: {
                                        id: id
                                    },
                                    dataType: 'json',
